...
<div class="col-4">
    <div class="card">
        <div class="card-body">
            <form method="post" action="{{ route('admin#categoryUpdate', $updateCategory['category_id']) }}">
                @csrf
                <div class="mb-3">
                  <label for="exampleInputEmail1" class="form-label">Category Name</label>
                  <input type="text" name="categoryName" value="{{ old('categoryName',$updateCategory['title']) }}" class="form-control" id="exampleInputEmail1" placeholder="Enter your category name..." aria-describedby="emailHelp">
                  @error('categoryName')
                ...
                <div class="mb-3">
                 ...
                 <textarea  type="text" name="descriptionName" value=""  class="form-control" placeholder="Enter Your Description..." >{{ old('descriptionName',$updateCategory['description']) }}</textarea>
                 @error('descriptionName')
                 <div class="text-danger">{{ $message }}</div>
                 @enderror
                </div>
                <button type="submit" class="btn btn-primary">Update</button>
                <a href="{{ route('admin#category') }}"><button type="button" class="btn btn-dark text-white float-end">Create</button></a>
              </form>
        </div>
    </div>
</div>
